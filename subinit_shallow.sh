...
git submodule foreach --recursive 'git config remote.origin.fetch "+refs/heads/*:refs/remotes/origin/*"; git checkout -b $(basename $PWD);  git fetch origin $(basename $PWD) --filter=blob:none; git branch -u origin/$(basename $PWD); git config branch.$(basename $PWD).remote origin'
